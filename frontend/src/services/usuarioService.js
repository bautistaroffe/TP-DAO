...
export const usuarioService = {
  async obtenerUsuarios() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Error en el servidor (${response.status} ${response.statusText}): ${errorDetail}`
        );
      }
  ...
  async buscarUsuarioPorDNI(dni) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Error al buscar usuario por DNI (${response.status}): ${errorDetail}`
        );
      }
  ...
  async obtenerUsuarioPorId(id_usuario) {
    console.log(`[SERVICE] Llamando a GET para Usuario ID: ${id_usuario}`);
    try {
      ...
      if (!response.ok) {
        const errorDetail = await getErrorDetail(response);
        throw new Error(
          `Error al obtener el usuario ID ${id_usuario} (${response.status}): ${errorDetail}`
        );
      }
      // Si es 200 y OK, devuelve el usuario
      return await response.json();
  ...
  async crearOActualizarUsuario(id_usuario, datosPayload, method) {
    ...
    console.log(
      `[SERVICE] Llamando a ${method} en URL: ${url} con payload:`,
      datosPayload
    );
    ...
    try {
      ...
      console.warn(
        `Operación ${method} exitosa pero falló la lectura del JSON de respuesta. Asumiendo éxito.`,
        e
      );
      return {};
    }
  ...
  async eliminarUsuario(id_usuario) {
    ...
    if (!response.ok) {
      const errorDetail = await response.text();
      throw new Error(
        `Error al eliminar el usuario: ${response.status} ${response.statusText} - ${errorDetail}`
      );
    }
    return true;
  },
